...
# def minimax(pile, is_maximizing, memo={}):
#     # is the dictionary created here?
#     # does it get reseted every time I used it? 
#     if (pile, is_maximizing) in memo:
#         return memo[(pile, is_maximizing)]
#             # this is the key for the appropriate result

    
#     if pile == 0:
#         result = (1 if not is_maximizing else -1, None)
#     else:
#         if is_maximizing:
#             result = max(
#                 (minimax(pile - take, False, memo)[0], take) # what does [0] mean?
#                 for take in range(1, min(pile, 3) + 1)
#             )
#         else:
#             result = min(
#                 (minimax(pile - take, True, memo)[0], take)
#                 for take in range(1, min(pile, 3) + 1)
#             )

#     memo[(pile, is_maximizing)] = result
#     return result
